...
class NowPlayingVideoWidget extends StatefulWidget {
  final String url;
  final double height;
  const NowPlayingVideoWidget({    Key? key, required this.url,    required this.height,}) : super(key: key);
  @override
...
class _NowPlayingVideoWidgetState extends State<NowPlayingVideoWidget> {
  ...
  void init() async {
    ...
    chewieController = ChewieController(
      videoPlayerController: videoPlayerController!,
      autoInitialize: true,      materialProgressColors:
    ChewieProgressColors(playedColor: AppColors.facebookBlue),
  ...
  Widget build(BuildContext context) {
    return SizedBox(
    height: widget.height.h,      width: double.infinity,
    child: (videoPlayerController == null)? Center(child: Shimmer.fromColors(  baseColor: AppColors.lightGray,
      highlightColor: AppColors.gray,  child: Container(
        height: double.infinity,
        width: double.infinity,    decoration: BoxDecoration(
        color: Colors.black,      borderRadius: BorderRadius.circular(8.0.r),
      ),  ),
